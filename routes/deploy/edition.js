    ...
    component.find({}, (err, data) => {
        if (err) {
            ...
            if (query.idDeployment) {
                ...
                if (newData.length !== project.length) {
                    let newArr = []
                    project.forEach(item => {
                        let arr = data.filter(each => {
                            return item.key === each.key;
                        });
                        newArr.push(arr);
                    });
                    newArr.forEach(item => {
                        let oneItem = item.filter(each => {
                            return each.idDeployment !== query.idDeployment
                        });
                        if (oneItem.length === item.length) {
                            newData.push(oneItem[oneItem.length - 1])
                        };
                    });
                };
                data = newData;
...
router.post('/add', (req, res, next) => {
    ...
    body.isHistory = body.isHistory ? body.isHistory : 'yes'
    ...
    if (body.key) {
        ...
        updateObj.projectName = body.projectName
    }
    //更新数据
    component.updateMany(updateObj, { $set: { idDeployment: 'no' } }, (err, result) => {
        if (err) {
            res.json({ result: false, code: 500 });
            console.log('错误信息：', err);
        } else {
...
router.post('/transfer', (req, res, next) => {
    let body = req.body;
    if (body.root && body.version && body.catalog && body.key && body.bid) {
        let SOURCES_DIRECTORY = './backups/' + body.root + '/' + body.version + '/' + body.catalog;  //源目录
        let _dirname = './www/' + body.root;  //目标目录
        // 删除根目录下的文件
        delDir(_dirname);//删除文件夹
        ...
        checkDirectory(SOURCES_DIRECTORY, _dirname, copy);
        // 变换状态
        component.updateMany({ key: body.key }, { $set: { idDeployment: 'no' } }, (err, result) => {
            if (err) {
                res.json({ result: false, code: 500 });
                console.log('错误信息：', err);
            } else {
                component.updateMany({ bid: body.bid }, { $set: { idDeployment: 'yes' } }, (err, result) => {
                    if (err) {
                        res.json({ result: false, code: 500 });
                        console.log('错误信息：', err);
                    } else {
...
router.post('/update', (req, res, next) => {
    ...
    component.updateMany(obj, { $set: body }, (err, data) => {
        if (err) {
